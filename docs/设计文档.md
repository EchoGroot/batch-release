...
+ 部署完成时，会将新版本的podTemplate记录到anno current-stable-reversion-raw，同时将上一个current-stable-reversion-raw
  记录到last-stable-reversion-raw
+ 部署完成后回滚
    + 使用last-stable-reversion-raw里的数据进行发布
+ 部署期间回滚
    + 使用current-stable-reversion-raw里的数据进行发布

##### 发布时扩缩容

+ 优先处理扩缩容
+ 按照当前新旧版本比例进行扩缩容，避免新版本太多引发线上风险
